...
class VisitUseCasesTest {
    ...
    fun attemptCheckInTest() {
        ...
        val building = Building(
            buildingEntity.id!!, buildingEntity.buildingName!!, buildingEntity.address,
        ...
        `when`(mockUserRepo.setCheckedInBuilding(userEntity.id!!, "1")).thenReturn(
            Single.just(
                userEntity
            )
        )
        `when`(mockVisitRepo.create(userEntity.id!!, buildingEntity.id!!)).thenReturn(
            Single.just(
                visitEntity
            )
        )
        `when`(mockBuildingRepo.incrementNumPeople(buildingEntity.id!!, 1.0)).thenReturn(
            Single.just(
                buildingEntity
            )
        )
        `when`(mockBuildingUseCases.getBuildingInfoById(buildingEntity.id!!)).thenReturn(
            Single.just(
                building
            )
        )
        `when`(mockVisitRepo.runCheckInTransaction(userEntity.id!!, buildingEntity.id!!)).thenReturn(
            Single.just(
                visitEntity
            )
        )
    ...
    fun checkOutCorrectlyTest() {
        ...
        val checkOutVisitEntity = VisitEntity(
            visitEntity.id,
            userEntity.id,
            buildingEntity.id,
            visitEntity.checkIn,
        ...
        val visit = Visit(user, building, visitEntity.checkIn, visitEntity.checkOut)
        val buildingCheckedOut = Building(
            buildingEntity.id!!,
            buildingEntity.buildingName!!,
            buildingEntity.address!!,
            buildingEntity.capacity!!,
            buildingEntity.numPeople?.minus(1)!!,
            buildingEntity.qrCodeRef!!
        )
        val checkOutVisit = Visit(
            checkOutUser,
            buildingCheckedOut,
            visitEntity.checkIn,
            checkOutVisitEntity.checkOut
        )
        `when`(mockUserUseCases.getCurrentlyLoggedInUser()).thenReturn(Single.just(user))
        ...
        visitUseCases.checkOut(buildingEntity.id!!).test().assertComplete().assertValue(
            checkOutVisit
        )
    }
    ...
    fun logInViewModelTest() {
        ...
        val mockLoginViewModel = LoginViewModel(mockAuthRepo)
        mockLoginViewModel.login("tommytrojan@usc.edu", "fightOn")
    ...
    fun managerHomeViewModelTest() {
        ...
        val mockManagerViewModel = ManagerProfileViewModel(mockAuthRepo)
